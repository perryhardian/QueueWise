...
export class QueuesService {
  ...
  async getBusinessQueue(businessId: string) {
    const queue = await this.prisma.queue.findFirst({
      ...
      include: {
        business: true,
        entries: {
          where: { status: { in: activeQueueEntryStatuses } },
  ...
  async getMerchantDashboard(userId: string, queueId: string) {
    ...
    const entries = queue.entries.map((entry) =>
      this.toMerchantQueueEntry(entry),
    );
    const waitingStatuses: QueueEntryStatus[] = [
    ...
    const waitingCount = queue.entries.filter((entry) =>
      waitingStatuses.includes(entry.status),
    ).length;
    const checkedInCount = queue.entries.filter(
      (entry) => entry.status === QueueEntryStatus.CHECKED_IN,
    ).length;
    const completedCount = queue.entries.filter(
      (entry) => entry.status === QueueEntryStatus.COMPLETED,
    ).length;
    ...
    return {
      ...
      business: {
        id: queue.business.id,
        name: queue.business.name,
        address: queue.business.address,
      },
      ...
      estimatedWaitingTimeMinutes: estimateWaitMinutes(
        waitingCount,
        queue.averageServiceTimeMinutes,
      ),
      entries,
    };
  }
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      const queue = await tx.queue.create({
        data: {
          ...
          nextSequence: 1,
          averageServiceTimeMinutes: dto.averageServiceTimeMinutes ?? 10,
          openedAt: new Date(),
        },
  ...
  async assertMerchantOwnsQueue(userId: string, queueId: string) {
    const queue = await this.prisma.queue.findUnique({
      where: { id: queueId },
      include: { business: { include: { merchant: true } } },
    });
    if (!queue) throw new NotFoundException('Queue not found');
    if (queue.business.merchant?.userId !== userId)
      throw new ForbiddenException('You do not manage this queue');
    return queue;
  }
  ...
  toQueueStatus(queue: QueueWithEntries) {
    const peopleWaiting = queue.entries.filter(
      (entry) => entry.status !== QueueEntryStatus.SERVING,
    ).length;
    return {
      queue: {
